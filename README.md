<h1 align="center">Hi 👋, I'm Anton Tretiak</h1>
<h3 align="center">Creative and goal-oriented Shopify Developer</h3>

💼 Specializing in **Shopify and Shopify Plus development (Theme Development, Checkout UI Extensions, Shopify Functions etc.), and front-end development (React.js, Liquid, JS)**  

📫 Reach me at **tretiak.ap@gmail.com**

<h3 align="left">Connect with me:</h3>
<p align="left">
<a href="https://www.linkedin.com/in/tretiak-ap/" target="blank"><img align="center" src="https://raw.githubusercontent.com/rahuldkjain/github-profile-readme-generator/master/src/images/icons/Social/linked-in-alt.svg" alt="anton tretiak" height="30" width="40" /></a>
<a href="https://instagram.com/anton.trettiak" target="blank"><img align="center" src="https://raw.githubusercontent.com/rahuldkjain/github-profile-readme-generator/master/src/images/icons/Social/instagram.svg" alt="anton.trettiak" height="30" width="40" /></a>
<a href="https://t.me/antontrettiak" target="blank"><img align="center" src="https://cdn3.iconfinder.com/data/icons/social-icons-33/512/Telegram-256.png" alt="telegram" height="30" width="30" /></a>
</p>

---

## 👨‍💻 About Me


I’m a highly skilled **Shopify Developer** with deep eCommerce experience and a strong focus on creating high-performing, visually polished, and conversion-driven online stores.  
I specialize in building custom Shopify themes, optimizing performance, and delivering tailored solutions that align with unique brand goals.

Over the past years, I’ve successfully developed, launched, and optimized multiple Shopify stores with an emphasis on clean UI/UX, responsiveness, and scalable architecture.

---

## 🔧 What I Do


### **Shopify Expertise**
- Shopify & Shopify Plus development  
- Custom theme development & UI/UX improvements  
- Shopify 2.0 features, metafields, and metaobjects  
- Custom checkout integrations  
- Store setup, optimization, and ongoing support  
- API integrations (REST, GraphQL)  
- Payment gateways & marketing tools (Klaviyo, Mailchimp)  

### **Technical Skills**
- **Front-end:** JavaScript, TypeScript, React.js, jQuery  
- **Back-end:** Node.js  
- **Other Tools:** Git, Webpack, Figma, Photoshop, Adobe XD
- **Best practices:** Performance optimization, SEO essentials, clean and maintainable code  

---

## 🚀 My Approach


I focus on understanding each client’s business needs and translating them into scalable, user-friendly Shopify solutions.  
My work philosophy is built on:

- Clean, maintainable, and bug-free code  
- Clear communication and transparency  
- Efficient problem-solving  
- Attention to both technical and business goals  

Let’s build something exceptional together!
